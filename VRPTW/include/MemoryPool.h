...
{
template <typename T> using IteratorListT = std::list<T *>::iterator;

template <typename T> using referenceT = std::reference_wrapper<T>;

#define DefRefW_T(T) using RefW_##T = std::reference_wrapper<T>;

template <typename T> class ProxT_notUsed
{
  public:
...
template <typename T> class Pool
{
  ...
  public:
    ...
    void startPool(const int bucketSizePage = 1, const int vetDelSizePage = 4)
    {
        ...
        assertm(posix_memalign((void **)&p_tDelT, pageSize, vetDelSizePage * bucketSize),
                "posix_memalign");
    ...
    T *getT()
    {
        ...
        if(proxT.iterator == listT.end())
        {
            ...
            // std::cout<<"Net bucket\n";
            T *p_tNew = nullptr;
            assertm(posix_memalign((void **)&p_tNew, pageSize, bucketSize),
                    "Bay more memory!");
