...
inline __attribute__((always_inline)) int getIndex(int i, int j, int numClie)
{
    return i * numClie + j;
}

inline __attribute__((always_inline)) void
reverseIndex(int index, int numClie, int &i, int &j)
{
    i = index / numClie;
    j = index % numClie;
}
...
class VrpLabelingSubProb : public DW_DecompNS::SubProb
{
  public:
    ...
    Vector<std::unique_ptr<LabelingAlgorithmNS::SortRoute>>
        vetSortRoute; // kepes all generated routes
    boost::unordered_multiset<LabelingAlgorithmNS::HashSortRoute>
        setSortRoute; // Kepes the hash of all route
    ...
    VrpLabelingSubProb();
    explicit VrpLabelingSubProb(InstanceVRPTW_NS::InstanceVRPTW     &instVrpTw,
                                double                               startDis,
    ...
    bool checkEnumeratedRoutesMid(Eigen::RowVectorXd &vetRowPi,
                                  Eigen::MatrixXd    &matColX,
                                  int                &numSol);
    void convertRouteIntoLabel(const TestNS::Route        &route,
...
class CapacityCut : public BranchAndPriceNS::RobustCutGenerator
{
  ...
  public:
    ...
    CapacityCut(InstanceVRPTW_NS::InstanceVRPTW &instVrpTw,
                int                              dim_,
                int                              maxNoOfCuts_,
                double                           eps);
    ~CapacityCut();
