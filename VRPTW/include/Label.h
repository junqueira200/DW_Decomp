...
class LabelCmp
{
  public:
    ...
    {
        // return doubleLess(l0->vetResources[0], l1->vetResources[0],
        ...
        return (l0->index < l1->index); // && l0->vetResources[0] < l1->vetResources[0];
                                        ...
                                        */
                                        /*
                                        if(MinHeap)
                                            return l0->HEAP_KEY < l1->HEAP_KEY;// && l0->vetResources[1] <
                                        l1->vetResources[1];                                 else                                 return l0->HEAP_KEY >
                                        l1->HEAP_KEY;
                                        */
    }
...
struct Bucket
{
  public:
    ...
    Bucket() { vetPtrLabel.resize(vetPtrLabelSize); }
    void flush()
    {
        if(NullFlush)
        {
...
{
  public:
    /// Access first the customer and after (i,j), where i is the component of the first
    /// resource and j
  ...
  public:
    ...
    INLINE
    int getIndexGraphBucket(int i, int j) { return i * vetNumSteps[1] + j; }
    ...
    {
        return vetMatBucketForward[getSecondDeposit()].mat(0, 0).sizeVetPtrLabel;
    }
