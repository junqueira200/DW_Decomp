    ...
    double                           startDist)
{
    ...
    labelingData = LabelingAlgorithmNS::LabelingData(
        vetStepSize, 2, instVrpTw->numClientes + 1, arrayMaxResorces);
...
void Cvrp_DecompLabelingNS::CvrpLabelingSubProb::iniConvConstr(GRBModel    &rmlp,
                                                               void        *data,
...
bool Cvrp_DecompLabelingNS::CvrpLabelingSubProb::checkEnumeratedRoutesFinal(
...
bool Cvrp_DecompLabelingNS::CvrpLabelingSubProb::checkEnumeratedRoutesMid(
    Eigen::RowVectorXd &vetRowPi,
    ...
    /*
     ...
     for(int l=0; l < numSol; ++l)
     {
         ...
         do
         {
             ...
             label->vetResources[0] += instVrpTw->matDist(lastCust, cliJ) -
     vetRowPi[cliJ+1]; label->vetResources[1] += instVrpTw->vetClieDem[cliJ];
     ...
     {
         std::cout<<"Error, bucket size("<<bucket.sizeVetPtrLabel<<") is different from
     "<<numSol<<"\n\n"; PRINT_EXIT();
     }
     ...
     for(const TestNS::Route& route:routeHash)
     {
         ...
         if(redCost < -DW_DecompNS::TolObjSubProb)
         {
             ...
             int pos = -1;
             Bucket* bucket = dominanceIntraBucket(instVrpTw->numClientes, label,
     labelingData, labelHeap, 2, instVrpTw->numClientes, pos);

             if(bucket != nullptr)
             {
                 //std::cout<<"Error!!\nRoute("<<route.vetRoute<<") have a negative
     reduced cost ("<<redCost<<
...
int Cvrp_DecompLabelingNS::CvrpLabelingSubProb::resolveSubProb(
    ...
    bool                                                    exact)
{
    ...
    */
    ...
    else
    {
        ...
        (FloatType)constPiValue, -1, true, maxDist, vetRedCostFT, exact, typeLabel,
        false);
...
void Cvrp_DecompLabelingNS::CvrpLabelingSubProb::convertRouteIntoLabel(
...
void Cvrp_DecompLabelingNS::CapacityCut::createInducedSubGraphArcs(
    int                          listSize,
