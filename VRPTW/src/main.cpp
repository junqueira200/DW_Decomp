...
int main(int argc, const char **argv)
{
    ...
    int edge_tail[] = {1, 2, 3, 4, 2, 3, 3, 4, 4};

    double edge_x[] = {0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5};

    CAPSEP_SeparateCapCuts(n_customers, demand, capacity, noOfEdeges, edge_tail,
    ...
    */
    ...
    try
    {
        ...
        instanciaG =
            Instance(instVrpTw.numClientes, instVrpTw.numClientes - 1, instVrpTw.numVeic);
        ...
        VrpLabelingSubProb vrpLabelingSubProb(
            instVrpTw, instVrpTw.sumDist(), vetMaxResources);
        // Cvrp_DecompLabelingNS::CvrpLabelingSubProb vrpLabelingSubProb(instVrpTw,
        // instVrpTw.sumDist());
        ...
        Index index = vrpLabelingSubProb.labelingData.getListOfIndexForMerge(label,
        vetMaxResouces);
        ...
        */
        ...
        auxVectors.vetPairSubProb.push_back(
            std::make_pair(0, instVrpTw.numClientes * instVrpTw.numClientes));
        // auxVectors.vetPairSubProb.push_back(std::make_pair(0, ((instVrpTw.numClientes *
        // (instVrpTw.numClientes-1))/2)));
        ...
        DW_DecompNS::DW_DecompNode decompNode(grbEnv,
                                              model,
                                              distVarA,
                                              (DW_DecompNS::SubProb *)&vrpLabelingSubProb,
                                              1,
                                              auxVectors);
        ...
        for(int i = 0; i < 100; ++i)
        {
            ...
            for(int j = 0; j < (int)best.vetRota.size(); ++j)
            {
                ...
                for(int t = 0; t < (int)rota.numPos - 1; ++t)
                {
                    ...
                    // std::cout<<"\t"<<cliI<<", "<<cliJ<<"\n";
                    int index = VrpTW_DecompLabelingNS::getIndex(
                        cliI, cliJ, instVrpTw.numClientes);
                    vetX[index] = 1;
                }
        ...
        /*        Eigen::MatrixXi matSol(instVrpTw.numVeic, instVrpTw.numClientes/2);
                ...
                {
                    if(vetSol[VrpTW_DecompNS::getIndex(i, j, instVrpTw.numClientes)] >=
           0.99)
                    {
                ...
                for(r=0; r < instVrpTw.numVeic; ++r)
                {
                    ...
                    while(i != 0)
                    {
                        ...
                        {
                            if(vetSol[VrpTW_DecompNS::getIndex(i, j,
        ...
        if((bool)std::fetestexcept(FE_OVERFLOW))
            std::cout << "Overflow flag after: " << (bool)std::fetestexcept(FE_OVERFLOW)
                      << std::endl;
        if((bool)std::fetestexcept(FE_UNDERFLOW))
            std::cout << "Underflow flag after: " << (bool)std::fetestexcept(FE_UNDERFLOW)
                      << std::endl;
...
void computeMeanMaxMin()
{
    ...
    if(size % 2 == 0)
    {
        median = (vetValueOfReducedCostsG[size / 2] +
                  vetValueOfReducedCostsG[(size / 2) + 1]) /
                 2.0;
    }
    else
        median = vetValueOfReducedCostsG[size / 2];
