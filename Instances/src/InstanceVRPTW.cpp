...
InstanceVRPTW_NS::InstanceVRPTW::InstanceVRPTW(int numClie_)
{
...
double InstanceVRPTW_NS::InstanceVRPTW::sumDist()
{
    double dist = 0.0;
...
int InstanceVRPTW_NS::InstanceVRPTW::sumDem()
{
    int sum = 0;
    for(int i = 0; i < numClientes; ++i)
        sum += vetClieDem[i];
...
void InstanceVRPTW_NS::leInstanciaSalomon(const std::string &strFile,
                                          InstanceVRPTW     &instVrpTw)
{
    ...
    instVrpTw = InstanceVRPTW(numClie + 1);
    ...
    for(int i = 0; i < numClie + 1; ++i)
    {
        for(int j = i + 1; j < numClie + 1; ++j)
        {
            ...
            double distI_J = calculateDistance(matData(i, XCoord),
                                               matData(i, XCoord),
                                               matData(j, XCoord),
                                               matData(j, YCoord));
        ...
        std::cout << i << ": [" << instVrpTw.vetClieTime[i].readyTime << ";"
                  << instVrpTw.vetClieTime[i].dueTime << "]; servTime("
                  << instVrpTw.vetClieTime[i].servTime << "); dem("
                  << instVrpTw.vetClieDem[i] << ")\n";
    }
...
void InstanceVRPTW_NS::leInstanciaAugerat(const std::string &strFile,
                                          InstanceVRPTW     &instCvrp)
{
    ...
    instCvrp = InstanceVRPTW(numClientes + 1);
    ...
    for(int i = 0; i < numClientes + 1; ++i)
    {
        ...
        {
            double temp =
                (sqrt(pow(vetorX[i] - vetorX[j], 2) + pow(vetorY[i] - vetorY[j], 2)));
...
double InstanceVRPTW_NS::calculateDistance(double x1, double y1, double x2, double y2)
{
...
double InstanceVRPTW_NS::somaDist(const InstanceVRPTW &instVrpTw)
{
    double dist = 0.0;
...
void InstanceVRPTW_NS::getSubInstancia(int numClientes, InstanceVRPTW &instVrpTw)
{
