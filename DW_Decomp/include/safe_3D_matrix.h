...
template <typename T, bool C = true> class Vector3D : public std::vector<T>
{
  private:
    size_t n = 0;
    size_t m = 0;
  ...
  public:
    explicit Vector3D(size_t n_, size_t m_, size_t p_)
        : std::vector<T>(n_ * m_ * p_), n(n_), m(m_), p(p_)
    {
    }

    Vector3D(size_t n_, size_t m_, size_t p_, const T &value)
        : std::vector<T>(n_ * m_ * p_, value), n(n_), m(m_), p(p_)
    {
    }
    ...
    Vector3D(const Vector3D<T, C> &temp)
        : n(temp.n), m(temp.n), p(temp.p), std::vector<T>(temp.n * temp.m * temp.p)
    {
                                                               ...
                                                               size_t indexK) const
    {
        ...
        if constexpr(C)
        {
            ...
            if(indexI >= n)
            {
                std::cout << "Erro indice i: " << indexI
                          << " esta errado para matrix de tam " << n << " x " << m
                          << " x " << p << "\n";
                throw std::out_of_range("");
            }
            ...
            if(indexK >= p)
            {
                ...
                std::cout << "Erro indice k: " << indexK
                          << " esta errado para vetor de tam " << n << " x " << m << " x "
                          << p << "\n";
                throw std::out_of_range("");
            }
    ...
    virtual inline __attribute__((always_inline)) T &
    get(size_t indexI, size_t indexJ, size_t indexK)
    {
        ...
        if(indexK >= p)
        {
            ...
            std::cout << "Erro indice k: " << indexK << " esta errado para vetor de tam "
                      << n << " x " << m << " x " << p << "\n";
            throw std::out_of_range("");
        }
