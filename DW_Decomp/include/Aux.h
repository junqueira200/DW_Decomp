...
#define PRINT_EXIT()                                                                     \
    std::cout << "  FILE: " << __PRETTYFILE__ << "  FUNC: " << __PRETTY_FUNCTION__       \
              << "  LINHA: " << __LINE__ << "\n";                                        \
    exit(-1);
#define PRINT_THROW()                                                                    \
    std::cout << "  FILE: " << __PRETTYFILE__ << "  FUNC: " << __PRETTY_FUNCTION__       \
...
template <typename T, int option> class TempSpMatPrint
{
  public:
...
template <typename T, int option> class TempSpVetPrint
{
  public:
...
{
    s << static_cast<const Eigen::SparseMatrixBase<Eigen::SparseMatrix<T, option>> &>(
        temp.m);
    return s;
}
...
{
    s << static_cast<const Eigen::SparseMatrixBase<Eigen::SparseVector<T, option>> &>(
        temp.m);
    return s;
}
...
doubleLess(double a, double b, double ep = std::numeric_limits<double>::epsilon())
{
    return (a - b) < -ep;
}

inline __attribute__((always_inline)) bool
doubleLess(float a, float b, float ep = std::numeric_limits<float>::epsilon())
{
    return (a - b) < -ep;
}
...
inline __attribute__((always_inline)) bool
doubleGreater(float a, float b, float ep = std::numeric_limits<float>::epsilon())
{
    return (a - b) > ep;
}
...
inline __attribute__((always_inline)) bool
doubleEqual(float a, float b, double ep = std::numeric_limits<double>::epsilon())
{
    // return std::fabs(a-b) < ep;
...
inline __attribute__((always_inline)) bool
doubleGreaterEqual(double a, double b, double ep = std::numeric_limits<double>::epsilon())
{
    return doubleGreater(a, b, ep) || doubleEqual(a, b, ep);
}

inline __attribute__((always_inline)) bool
doubleGreaterEqual(float a, float b, float ep = std::numeric_limits<float>::epsilon())
{
    return doubleGreater(a, b, ep) || doubleEqual(a, b, ep);
}

inline __attribute__((always_inline)) bool
doubleLessEqual(double a, double b, double ep = std::numeric_limits<double>::epsilon())
{
    return doubleLess(a, b, ep) || doubleEqual(a, b, ep);
}

inline __attribute__((always_inline)) bool
doubleLessEqual(float a, float b, float ep = std::numeric_limits<double>::epsilon())
{
    return doubleLess(a, b, ep) || doubleEqual(a, b, ep);
}
