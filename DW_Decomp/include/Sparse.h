...
template <typename T> int64_t binarySearch(const Vector<T> &vet, T val, int64_t size)
{
...
template <typename T> class SparseMatLin
{
  public:
    ...
    // Warning: adds a new element if there isn't exist; Expensive if it called out of
    // order in j!
    T &operator()(int64_t i, int64_t j)
    {
#ifdef VAR_VECTOR_SANITY_CHECK
        assertm(
            i >= numLin, "i(" << i << ") is out of range in numLine(" << numLin << ")");
        assertm(
            j >= numCol, "j(" << j << ") is out of range in numCol(" << numCol << ")");
#endif
    ...
    SparseMatLin(const SparseMatLin<T> &mat)
    {
        ...
        for(int64_t i = 0; i < numLin; ++i)
        {
            ...
            assertm(mat.vetVetData[i]->size() != mat.vetVetInd[i]->size(),
                    "size(vetData) != size(vetInd)!\n");
...
template <typename T> class SparseMatCol
{
  public:
    ...
    int64_t getCapCol() const { return capCol; }
