...
void DW_DecompNS::getSparseMatModel(GRBModel                                     &model,
 ...
 * @param mestre                Modelo com as restricoes do mestre e a func obj do
 * problema original
 * @param custoVarA             Custo das variaveis artificiais, para inicializar a CG
 * @param vetPairSubProb        Vetor de pair por subproblema, cada pair representa o
 * indice do inicio das variaveis e o seu tamanho
 * @param subProb               Ptr para class SubProb
 * @param data                  Ptr para void* eh passado para os metodos de @param
 * subProb
                           ...
                           const int                                numSubProb)
{
    ...
    for(int i = 0; i < numSubProb; ++i)
    {
        ...
        funcGetSubMatrix(matA,
                         vetSubMatA[i],
                         numConstrsMestre,
                         vetPairSubProb.at(i).second,
                         vetPairSubProb.at(i).first,
                         0);
    }
    ...
    std::vector<std::unique_ptr<Eigen::VectorXd>>
                    vetVarLambda; // Vetores gerados pelos subproblemas
    ...
    rmlpConstrs = rmlp.addConstrs(
        vetLinExprRmlp, vetRmlpConstrsSense, vetRmlpRhs, vetStrConstrs, numConstrsMestre);
    delete[] rmlpConstrs;
    rmlp.update();
    ...
    auto funcGetSubProbCooef = [&](int k)
    {
        ...
        subProbCooef.segment(0, numVarSubProbK) =
            (vetC.segment(iniVarSubProbK, numVarSubProbK) -
             (vetRmlpDuals.segment(0, numVarSubProbK).transpose() * vetSubMatA[k])
                 .transpose())
                .transpose();
    };

    auto funcAddColumn = [&](Eigen::VectorXd       &vetCooef,
                             const Eigen::VectorXd &vetConvCooef,
                             const double           cost,
                             int                    k)
    {
        GRBColumn grbColumn;
    ...
    : ptrSubProb(ptrSubProb_)
{
    ...
    int numConstrsRmlp = info.numConstrsMaster +
                         ptrSubProb->getNumConvConstr(); // + int(vetSubProb.size());
    info.numVarRmlpPi = numConstrsRmlp;

    // std::cout<<"numVarRmlpPi: "<<info.numVarRmlpPi<<"\n";
    // std::cout<<"numConstrsMaster: "<<info.numConstrsMaster<<"\n\n";
    // std::cout<<"ptrSubProb->getNumConvConstr:
    // "<<ptrSubProb->getNumConvConstr()<<"\n\n";
    ...
    for(int i = 0; i < info.numConstrsMaster + info.numConstrsConv; ++i)
        vetVarLambdaCol.emplace_back(
            std::make_unique<Eigen::VectorXd>(info.numVarMaster));
...
DW_DecompNS::StatusProb DW_DecompNS::DW_DecompNode::columnGeneration(AuxData &auxVect)
{
    ...
    VectorI vetLastIt(
        info.numConstrsOrignalProblem + info.numConstrsConv,
        -1); // The last iteration the artificial variable is different from 0
    ...
    {
        std::cout << "ERROR\n";
        std::cout << "LP: " << uRmlp->get(GRB_IntAttr_NumVars)
                  << "; vet: " << vetVarLambdaCol.size() << "\n";
        PRINT_DEBUG("", "");
        throw "ERROR";
    }
    ...
    while(subProbCustR_neg)
    {
        ...
        // std::cout<<"LP: "<<uRmlp->get(GRB_IntAttr_NumVars)<<"; vet:
        // "<<vetVarLambdaCol.size()<<"\n";
        ...
        */
        ...
        if(phaseStatus == PhaseStatus::PhaseStatusBigM && itCG > 0)
        {
            ...
            for(int i = 0; i < info.numConstrsOrignalProblem + info.numConstrsConv; ++i)
            {
                if(!doubleEqual(vetVar[i].get(GRB_DoubleAttr_X), 0.0, 1E-5))
                {
                    ...
                    std::cout << "set " << i << "_"
                              << vetVar[i].get(GRB_StringAttr_VarName) << "("
                              << vetVar[i].get(GRB_DoubleAttr_X)
                              << ") variable to: " << vetMult[i] * info.costA_Var << "\n";
        ...
        else if(phaseStatus == PhaseStatus::PhaseStatusTwoPhase)
        {
            ...
            {
                std::cout
                    << "All artificial variables are equal to zero!\nChanging to Column "
                       "Genration phase!\n\n";
                delete[] vetVar;
        ...
        // GRBVar *vetVar        = uRmlp->getVars();
        // double *vetRmlpLambda = uRmlp->get(GRB_DoubleAttr_X, vetVar,
        // uRmlp->get(GRB_IntAttr_NumVars));
        if(PrintDebug)
            std::cout << "UpdatePi\n";
        ...
        if(stabilization && !exactPi &&
           itCG > 0) // && phaseStatus == PhaseStatus::PhaseStatusColGen)
        {
            auxVect.vetRowRmlpSmoothPi =
                (1.0 - StabilizationAlpha) * auxVect.vetRowRmlpSmoothPi +
        ...
        */
        ...
        for(int k = 0; k < info.numSubProb; ++k)
        {
            ...
            for(int l = 0; l < numSol; ++l)
            {
                ...
                for(int i = 0; i < auxVect.vetPairSubProb[k].second; ++i)
                    vetSol[auxVect.vetPairSubProb[k].first + i] =
                        auxVect.matColX_solSubProb(i, l);
                ...
                if(info.numConstrsConv > 0)
                    auxVect.vetColCooef.segment(0, info.numConstrsConv) =
                        auxVect.vetColConvCooef;
                ...
                if(PrintDebug)
                {
                    ...
                    std::cout << "\tinfo.numConstrsMaster: " << (info.numConstrsMaster)
                              << "\n";
                    std::cout << "\tnumConstrsConv: " << info.numConstrsConv << "\n";
                    std::cout << "\tauxVect.vetColCooef.size: "
                              << auxVect.vetColCooef.size() << "\n";
                }

                auxVect.vetColCooef.segment(
                    info.numConstrsConv, (info.numConstrsMaster)) = matA * vetSol;
                ...
                if(phaseStatus != PhaseStatus::PhaseStatusColGen)
                {
                    ...
                    for(int i=1; i < info.numVarRmlpPi; ++i)
                    {
                        if(!doubleEqual(auxVect.vetColCooef[i], 0.0))
                        {
                            ...
                            std::cout<<"seting
                var("<<vetVar[i-1].get(GRB_StringAttr_VarName)<<") to 0\n";
                        }
                    }
                }
                */
        ...
        if(Stabilization == false && missPricing)
        {
            subProbCustR_neg = true;
            break;
        }
        ...
        */
        ...
        vetVarLambdaCol.size()
            ...
            if(!subProbCustR_neg && phaseStatus == PhaseStatus::PhaseStatusTwoPhase &&
        !allZero) return StatusSubProb_Inviavel;
    ...
    // std::cout<<"numConstrsOrignalProblem:
    // "<<info.numConstrsOrignalProblem+info.numConstrsConv<<"\n\n";
    GRBVar *vetVar = uRmlp->getVars();
    for(int i = 0; i < info.numConstrsOrignalProblem + info.numConstrsConv; ++i)
    {
...
void DW_DecompNS::DW_DecompNode::getSubProbCooef(int k, AuxData &auxVect)
{
    ...
    std::cout << "vetColSubProbCooef: "
              << auxVect.vetColSubProbCooef.segment(0, numVarSubProbK).transpose()
              << "\n\n";
}
...
void DW_DecompNS::DW_DecompNode::addColumn(const double cost, int k, AuxData &auxVect)
{
    ...
    grbColumn.addTerms(
        auxVect.auxVetCooef, vetRmlpConstr, info.numConstrsMaster + info.numConstrsConv);
...
void DW_DecompNS::DW_DecompNode::addColumnX(const double     cost,
                                            int              k,
                                            AuxData         &auxVect,
                                            Eigen::VectorXd &vetX)
{
    ...
    auxVect.vetColCooef.segment(info.numConstrsConv, info.numConstrsMaster) =
        matA * vetSol;
...
DW_DecompNS::DW_DecompNode::DW_DecompNode(const DW_DecompNS::DW_DecompNode &decomp)
{
    ...
    for(int i = 0; i < int(decomp.vetVarLambdaCol.size()); ++i)
        vetVarLambdaCol.push_back(
            std::make_unique<Eigen::VectorXd>(*decomp.vetVarLambdaCol[i]));
...
void DW_DecompNS::DW_DecompNode::getSolX()
{
    ...
    GRBVar *vetVar = uRmlp->getVars();
    double *vetRmlpLambda =
        uRmlp->get(GRB_DoubleAttr_X, vetVar, uRmlp->get(GRB_IntAttr_NumVars));
    double sum = 0.0;
...
DW_DecompNS::AuxData::~AuxData() { delete[] auxVetCooef; }
...
void DW_DecompNS::StabilizationData::getWeightSum(const double        alpha,
