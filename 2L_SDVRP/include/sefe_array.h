...
template <typename T, std::size_t n> class Array : public std::array<T, n>
{
  public:
    ...
    inline __attribute__((always_inline)) T &operator[](size_t index)
    {
        ...
        {
            std::cout << "Erro indice " << index << " esta errado para array de tam "
                      << std::array<T, n>::size() << "\n";
...
inline __attribute__((always_inline)) std::ostream &operator<<(std::ostream      &os,
                                                               const Array<T, n> &v)
{
