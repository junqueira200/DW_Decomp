...
{
    os << "[clie(" << estado.cliente << ") idVeic(" << estado.rotaId << ") pos("
       << estado.pos << ") incDist(" << estado.incDist << ")]";
    return os;
}
...
void melhorInsercao(SolucaoNS::Rota &rota,
                    int              clie,
                    int             &pos,
                    double          &inc,
                    double           alphaBin);
...
inline bool estadoComp(const Estado &e0, const Estado &e1)
{
    return e0.incDist < e1.incDist;
}
