...
class PlacementPointGenerator
{
  public:
    ...
    */
    ...
    /// Regular normal patterns according to Côté, J. F., & Iori, M. (2018). The
    ...
    DetermineRegularNormalPatternsX(int                          containerDx,
                                    int                          actualContainerDx,
                                    const std::vector<Cuboid *> &items);

    static boost::dynamic_bitset<>
    DetermineRegularNormalPatternsY(int                          containerDy,
                                    int                          actualContainerDy,
                                    const std::vector<Cuboid *> &items);

    static boost::dynamic_bitset<>
    DetermineRegularNormalPatternsZ(int                          containerDz,
                                    int                          actualContainerDz,
                                    const std::vector<Cuboid *> &items);

    static std::tuple<std::vector<int64>, std::vector<int64>, std::vector<int64>>
    DetermineMeetInTheMiddlePatterns(const Container     &container,
    ...
    static std::vector<boost::dynamic_bitset<>> DetermineMinimalMeetInTheMiddlePatterns(
        const Container                  &container,
        std::vector<Cuboid>              &items,
        MeetInTheMiddleMinimizationTarget minimizationTarget,
        Axis                              axis);
    ...
    DetermineUnitDiscretizationPoints(const Container     &container,
                                      std::vector<Cuboid> &items,
                                      Axis                 axis);
    static std::vector<boost::dynamic_bitset<>>
    GenerateRegularNormalPatterns(const Container     &container,
                                  std::vector<Cuboid> &items,
                                  Axis                 axis);

    static std::
        tuple<boost::dynamic_bitset<>, boost::dynamic_bitset<>, boost::dynamic_bitset<>>
        DetermineItemSpecificMeetInTheMiddlePatterns(const Container &container,
                                                     const std::vector<Cuboid *> &items,
                                                     const Cuboid                &itemI);

    static boost::dynamic_bitset<>
    DetermineRegularNormalPatterns(const Container             &container,
                                   const std::vector<Cuboid *> &items,
                                   const Cuboid                &itemI,
    ...
    /// Computing, 30(4), 646-661.
    static std::vector<boost::dynamic_bitset<>>
    DetermineReducedMeetInTheMiddlePatterns(std::vector<Cuboid> &items,
                                            const Container     &container,
                                            int                  threshold,
                                            Axis                 axis,
    ...
    static void DetermineSingleItemReducedLeftRightPatterns(
        int                          threshold,
    ...
    static void RemoveRedundantPatterns(
        std::vector<Cuboid>                  &items,
    ...
    static std::vector<boost::dynamic_bitset<>> GenerateMeetInTheMiddlePatterns(
        std::vector<Cuboid>                        &items,
        const Container                            &container,
    ...
    static std::vector<int64> ConvertPlacementBitsetToVector(
        const boost::dynamic_bitset<> &itemSpecificPatternBitset);
    static ItemPlacementPatterns ConvertPlacementBitsetToVector(
    ...
    ////static std::vector<int64> DetermineStartPointsFlatSpan(const std::vector<int64>&
    /// patterns); // Keep for reference.
    static std::vector<int64> DetermineEndPoints(const std::vector<int64> &patterns,
                                                 int                       dim,
                                                 int                       rotDim,
                                                 bool enableRotation);
    ...
    static std::vector<std::vector<int64>>
    GeneratePlacementPatternsBaseType(const Container     &container,
                                      std::vector<Cuboid> &items,
                                      PlacementPattern     placementPatternType,
                                      Axis                 axis);

    static std::
        unordered_map<Cuboid, ItemPlacementPatterns, HomogeneityHash, HomogeneityHash>
        GeneratePlacementPatterns(const Container     &container,
                                  std::vector<Cuboid> &items,
    ...
    static std::tuple<std::unordered_map<Cuboid,
    ...
    static void CountPlacementPoints(
