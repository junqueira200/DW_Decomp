...
class CpOptimizer
{
  public:
    ...
    IloArray<IloArray<IloBoolVarArray>>
        mRelativeDirections; // mRelativeDirections[i][j][direction], bool, 1
                             // if item i is placed relatively to item j in direction

    IloArray<IloBoolVarArray> mItemsOverlapsXY; // mItemsOverlapsXY[i][j], bool, items i
                                                // and j intersect in xy-plane
    IloArray<IloBoolVarArray>
        mSupportXY; // mSupportXY[i][j], bool, 1, if item i is supported by item j
                    // xy-plane ? -> items intersect AND item j is directly below item i
    IloArray<IloIntVarArray>
        mOverlapAreasXY; // mOverlapAreasXY[i][j], integer, size of intersection area in
...
void inline NameVars(IloIntVarArray &a, const char *base)
{
...
void inline NameVars(IloIntervalVarArray &a, const char *base)
{
    for(IloInt i = 0; i < a.getSize(); i++)
    {
        char name[100];
...
void inline NameVars(IloArray<IloIntVarArray> &a, const char *base)
{
    for(IloInt i = 0; i < a.getSize(); i++)
    {
...
void inline NameVars(IloArray<IloIntervalVarArray> &a, const char *base)
{
    for(IloInt i = 0; i < a.getSize(); i++)
    {
        for(IloInt j = 0; j < a[i].getSize(); j++)
