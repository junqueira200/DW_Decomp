/*
    ...
    Safe C++, Or How to Avoid Most Common Mistakes in C++ Code by Vladimir Kushnir,
   (OÕReilly). Copyright 2012 Vladimir Kushnir, ISBN 9781449320935. If you feel your use
   of code examples falls outside fair use or the permission given above, feel free to
...
class M_TrianngSup
{
    M_TrianngSup() = default;
};
class M_TrianngInf
{
    M_TrianngInf() = default;
};

// Wrapper around std::vector, has temporary sanity checks in the operators [].
template <typename T, typename M = M_Linha> class Matrix : public std::vector<T>
{
  private:
    size_t n = 0;
    size_t m = 0;
  ...
  public:
    ...
    Matrix(const Matrix<T, M> &temp)
        : n(temp.n), m(temp.n), std::vector<T>(temp.n * temp.m)
    {
        for(int i = 0; i < n * m; ++i)
    ...
    virtual inline __attribute__((always_inline)) T operator()(size_t indexI,
                                                               size_t indexJ) const
    {
    ...
    [[nodiscard]] virtual size_t inline __attribute__((always_inline))
    getNumLinhas() const
    {
        return n;
    }
    ...
    inline __attribute__((always_inline)) bool ehTransposta() const
    {
        return (std::is_same_v<M, M_Coluna>);
    };
...
template <typename T, typename M = M_TrianngSup> class MatrixTriangular : public Matrix<T>
{
  ...
  public:
    ...
    inline __attribute__((always_inline)) bool ehTransposta()const{return
  (std::is_same_v<M,M_Coluna>);};
    ...
    inline __attribute__((always_inline)) auto getIterator(size_t indexI, size_t indexJ)
    {
        ...
        if(indexI >= n)
        {
            std::cout<<"Erro indice i: "<<indexI<<" esta errado para matrix de tam "<<n<<"
  x
  "<<m<<"\n"; throw std::out_of_range("");
        }
        ...
        if(indexJ >= m)
        {
            std::cout<<"Erro indice j: "<<indexJ<<" esta errado para matrix de tam "<<n<<"
  x
  "<<m<<"\n"; throw std::out_of_range("");
        }
  #endif
...
void multMatrix(const Matrix<T> &mat0, const Matrix<T,M> &mat1, Matrix<T> &matResult)
{
       ...
       mat0.getNumColunas() != mat1.getNumLinhas())
    {
        std::cout<<"Erro, num linhas("<<mat0.getNumLinhas()<<") da mat0 != "<<
                                                             "num
col("<<mat1.getNumColunas()<<") da mat1\n"; throw "ERRO";
    }

    if(matResult.getNumLinhas() != mat0.getNumLinhas() || matResult.getNumColunas() !=
mat1.getNumColunas())
    {
#if VAR_VECTOR_SANITY_CHECK
    ...
    {
        size_t i;
        #pragma omp parallel for simd firstprivate(numLinhas,numCol, mat0NumCol, mat0,
mat1) shared(matResult) private(i) for(i = 0; i < numLinhas; ++i)
        {
