...
InstanceNS::Instance::Instance(int numClientes_, int numItens_, int numVeiculos_)
    : matDist(numClientes_, numClientes_), matTempo(numClientes, numClientes),
      // vetItens(numItens_),
...

{
    ...
    assertm(numClientes > MaxNumOfCustorms,
            "Error, constant MaxNumOfCustorms is under estimate!\n");

    if(numClientes < 0 || numItens < (numClientes - 1))
    {
        assertm(true,
                "numClientes(" << numClientes << ") < 0 || numItens(" << numItens
                               << ") < numClientes-1(" << numClientes << ")");
    }
...
InstanceNS::Instance::Instance()
    : matDist(0, 0), matTempo(0, 0), vetItens(0), vetDemandaCliente(0),
      vetItemCliente(0, 0), matCliItensIniFim(0, 0), vetPesoItens(0)
...
void InstanceNS::read2dInstance(const std::string &strFile)
{
    ...
    // std::cout<<"numClientes: "<<numClientes<<"; numItens: "<<numItens<<" numVeic:
    // "<<numVeiculos<<"\n";
    ...
    file >> instanciaG.maxPayload >> instanciaG.vetDimVeiculo[0] >>
        instanciaG.vetDimVeiculo[1];

    // std::cout << "veicCap: " << instanciaG.veicCap << "; veicComprimento: " <<
    // instanciaG.vetDimVeiculo[0] << "; veicLargura: " << instanciaG.vetDimVeiculo[1] <<
    // "\n\n";
...
void InstanceNS::readOroloc3D(const std::string &strFile)
{
    ...
    */
    ...
    for(int i = 0; i < instanciaG.numClientes; ++i)
    {
        ...
        {
            std::printf("Error while creating mapCustomerToCustomer_id and "
                        "mapCustomer_idToCustomer to %d and %d\n",
                        customer,
                        customer_id);
            PRINT_DEBUGG("", "");
            throw "ERROR";
...
void InstanceNS::read3dInstance(const std::string &strFile)
{
    ...
    std::cout << "numClientes: " << numClientes << "; numItens: " << numItens
              << " numVeic: " << numVeiculos << "\n";
...
int InstanceNS::copiaItensCliente(int cliente, VectorI &vetItens)
{
    ...
    for(int i = instanciaG.matCliItensIniFim(cliente, 0);
...
int InstanceNS::copiaItensClientes(VectorI &vetClientes,
                                   int      tam,
                                   VectorI &vetItens,
                                   bool     push)
{
...
int InstanceNS::generateRandomListOfItems(int numItens, VectorI &vetItems)
{
    ...
    volumeVeiculo = instanciaG.vetDimVeiculo[0] * instanciaG.vetDimVeiculo[1] *
...
void InstanceNS::readOroloc3D2(const std::string &strFile)
{
    ...
    std::getline(file, trash); // i	x	y	Demand	ReadyTime	DueDate
                               // ServiceTime	DemandedMass	DemandedVolume
    ...
    */
    ...
    std::getline(file, trash); // Type	Length	Width	Height	Mass	Fragility
                               // LoadBearingStrength
    std::getline(file, trash);
