...
PlacementPointGenerator::DetermineMeetInTheMiddlePatterns(const Container     &container,
                                                          std::vector<Cuboid> &items)
{
    ...
    for(size_t i = 0; i < items.size(); ++i)
    {
        ...
        auto [placeableItemCoordinatesX,
              placeableItemCoordinatesY,
              placeableItemCoordinatesZ] =
    ...
    return std::make_tuple(std::move(placementPointsX),
                           std::move(placementPointsY),
                           std::move(placementPointsZ));
}

std::tuple<boost::dynamic_bitset<>, boost::dynamic_bitset<>, boost::dynamic_bitset<>>
PlacementPointGenerator::DetermineItemSpecificMeetInTheMiddlePatterns(
    ...
    const Cuboid                &itemI)
{
    ...
    // Meet-in-the-middle patterns should only be generated for one value of t. When t =
    // binDimension, meet-in-the-middle patterns are equal to regular normal patterns,
    // i.e., MiM_it = \mathcal{B}_i.
    meetInTheMiddlePointsX |=
        DetermineMeetInTheMiddlePatterns(container, items, itemI, container.Dx, Axis::X);
    meetInTheMiddlePointsY |=
        DetermineMeetInTheMiddlePatterns(container, items, itemI, container.Dy, Axis::Y);
    meetInTheMiddlePointsZ |=
        DetermineMeetInTheMiddlePatterns(container, items, itemI, container.Dz, Axis::Z);

    return std::make_tuple(std::move(meetInTheMiddlePointsX),
                           std::move(meetInTheMiddlePointsY),
                           std::move(meetInTheMiddlePointsZ));
}

std::vector<ItemPlacementPatternsBitset>
PlacementPointGenerator::DetermineMinimalMeetInTheMiddlePatterns(
    const Container                  &container,
    std::vector<Cuboid>              &items,
                                                           ...
                                                           Axis                 axis)
{
    ...
    for(size_t i = 0; i < items.size(); ++i)
    {
        ...
        for(size_t p = containerDimension + 1 - minimumItemDimension;
            p < unitDiscretizationPoints.size();
            p++)
        {
            unitDiscretizationPoints.reset(p);
        }

        itemSpecificUnitDiscretizationPoints.push_back(
            std::move(unitDiscretizationPoints));
    }
    ...
    Axis                              axis)
{
    ...
    for(size_t i = 0; i < items.size(); ++i)
    {
        ...
        boost::dynamic_bitset<> regularNormalPattern =
            DetermineRegularNormalPatterns(container,
                                           filteredItems,
                                           itemI,
    ...
    {
        // Is it guaranteed that the reduced set is also minimal for tMin (tMin is
        // determined without preprocessing)? Probably not.
        // TODO.Design: preprocessing could also be integrated into
        // GenerateMinimalMeetInTheMiddlePatternsX(). Could it be integrated even earlier
        // to guarantee to find tMin?
        itemSpecificMeetInTheMiddlePoints =
            DetermineReducedMeetInTheMiddlePatterns(items,
                                                    container,
                                                    tMin,
                                                    axis,
                                                    enablePreprocessingStep1,
                                                    enablePreprocessingStep2);
    }
    else
    {
        itemSpecificMeetInTheMiddlePoints = GenerateMeetInTheMiddlePatterns(
            items, container, regularNormalPatterns, tMin, axis);
    }

    return itemSpecificMeetInTheMiddlePoints;
}

/// Preprocessing steps according to Côté, J. F., & Iori, M. (2018). The
/// meet-in-the-middle principle for cutting and packing problems. INFORMS Journal on
/// Computing, 30(4), 646-661.
std::vector<boost::dynamic_bitset<>>
PlacementPointGenerator::DetermineReducedMeetInTheMiddlePatterns(
    std::vector<Cuboid> &items,
    const Container     &container,
    int                  threshold,
    Axis                 axis,
    ...
    bool                 enablePreprocessingStep2)
{
    ...
    int separationThreshold =
        std::ceil((container.Dimension(axis) - minOverallDimension) / 2.0);
    ...
    // Determine left and right patterns. If Proposition 5 is enabled, with additionally
    // reduced items sets. If it is disabled with the standard item set resp. the standard
    // meet-in-the-middle procedure, cp. Algorithm 2 in the paper.
    for(size_t i = 0; i < items.size(); i++)
    {
        ...
        for(size_t k = 0; k < items.size(); k++)
        {
            if(k == i)
            {
                // Do not filter k == minimalItemIndex if Proposition 5 is disabled, s.t.
                // doublyFilteredItems is only reduced by i, as in the normal procedure.
                continue;
            }
        ...
        itemSpecificPlacementPointsRightPrime.emplace_back(
            std::move(placementPointsRightPrime));
    }
    ...
    for(size_t i = 0; i < items.size(); i++)
    {
        ...
        const boost::dynamic_bitset<> &placementPointsRightPrime =
            itemSpecificPlacementPointsRightPrime[i];
        boost::dynamic_bitset<> placementPointsRight(container.Dimension(axis) + 1);

        // Placement points placementPointsRightPrime (R'_it) are generated as a left-side
        // pattern. But, we want to obtain the corresponding right-side pattern
        // (placementPointsRight, R_it). So, R'_it must be mirrored to obtain R_it.
        for(size_t p = 0; p < placementPointsRightPrime.size(); p++)
        {
            if(placementPointsRightPrime[p])
            {
                placementPointsRight.set((size_t)container.Dimension(axis) -
                                         (size_t)minItemDimensionI - p);
            }
        }

        itemSpecificPlacementPointsRight.emplace_back(std::move(placementPointsRight));

        // Then build the union of placementPointsLeft (L_it) and R_it to obtain the set
        // of meet-in-the-middle points (M_it).
        itemSpecificMeetInTheMiddleSets.emplace_back(std::move(
            itemSpecificPlacementPointsLeft[i] | itemSpecificPlacementPointsRight[i]));
    }
    ...
    // Necessary since right placement points may be changed in
    // DetermineEnlargedItemDimensionsRight()
    for(size_t i = 0; i < items.size(); i++)
    {
        itemSpecificMeetInTheMiddleSets[i] =
            itemSpecificPlacementPointsLeft[i] | itemSpecificPlacementPointsRight[i];
    }

    // Proposition 7
    RemoveRedundantPatterns(
        items, itemSpecificModifiedItemDimensions, itemSpecificMeetInTheMiddleSets);
    ...
    const std::vector<Cuboid *> &filteredItems)
{
    ...
    else
    {
        ...
        // Example 1.
        // Two items with dimensions = {0: 2, 1: 5}. Assume t = container.Dx = 20, so t >
        // separationThreshold = 9. A feasible solution must exist (for any t). Because t
        // = container.Dx, M_i == \mathcal{B}_i. When itemI = 0: minimalItemIndex = 0,
        // items = {1}, filteredItems = {1},
        //                  L_0 = {0, 5}, R'_0 = {} because container.Dx -
        //                  minSelectedItemDimension - t <= 0.
        // When itemI = 1:  minimalItemIndex = 0, items = {0}, filteredItems = {},
        //                  L_1 = {0, 2}, R'_1 = {}.
        // A solution exists that satisfies the MiM principle, e.g. {0: [0, 2], 1: [2,
        // 7]}.
        //
    ...
    case Axis::X:
        ...
        placementPointsRightPrime = DetermineRegularNormalPatternsX(
            container.Dimension(axis) - minSelectedItemDimension - threshold,
            container.Dimension(axis),
            filteredItemsA);
        break;
    case Axis::Y:
...
void PlacementPointGenerator::DetermineEnlargedItemDimensionsLeft(
    ...
    const std::vector<boost::dynamic_bitset<>>
        &preliminaryItemSpecificMeetInTheMiddleSets)
{
    for(size_t k = 0; k < items.size(); k++)
    {
        const Cuboid &itemK = items[k];
        ...
        const boost::dynamic_bitset<> &placementPointsLeft =
            itemSpecificPlacementPointsLeft[k];
        if(placementPointsLeft.count() == 0)
            continue;

        for(size_t p = 0, n = placementPointsLeft.size() - minSelectedItemDimension;
            p < n;
            p++)
        {
            ...
            for(size_t i = 0; i < items.size(); i++)
            {
                ...
                const boost::dynamic_bitset<> &meetInTheMiddlePointsI =
                    preliminaryItemSpecificMeetInTheMiddleSets[i];

                for(size_t s = 0,
                           m = meetInTheMiddlePointsI.size() - minSelectedItemDimensionI;
                    s < m;
                    s++)
                {
        ...
        &preliminaryItemSpecificMeetInTheMiddleSets)
{
    for(size_t k = 0; k < items.size(); k++)
    {
        ...
        std::vector<int> &modifiedItemDimensionsK = itemSpecificModifiedItemDimensions[k];
        boost::dynamic_bitset<> &placementPointsRight =
            itemSpecificPlacementPointsRight[k];
        ...
        for(size_t p = 0; p < placementPointsRight.size() - minSelectedItemDimension; p++)
        {
            ...
            for(size_t i = 0; i < items.size(); i++)
            {
                ...
                const boost::dynamic_bitset<> &meetInTheMiddlePointsI =
                    preliminaryItemSpecificMeetInTheMiddleSets[i];
                std::vector<int> &modifiedItemDimensionsI =
                    itemSpecificModifiedItemDimensions[i];

                for(size_t s = 0;
                    s < meetInTheMiddlePointsI.size() - minSelectedItemDimensionI;
                    s++)
                {
                    if(!meetInTheMiddlePointsI[s])
                    {
    ...
    std::vector<boost::dynamic_bitset<>> &itemSpecificPlacementPoints)
{
    for(size_t k = 0; k < items.size(); k++)
    {
        ...
        const std::vector<int> &modifiedItemDimensionsK =
            itemSpecificModifiedItemDimensions[k];
...
std::vector<boost::dynamic_bitset<>>
PlacementPointGenerator::GenerateMeetInTheMiddlePatterns(
    std::vector<Cuboid>                        &items,
    const Container                            &container,
    ...
    Axis                                        axis)
{
    ...
    for(size_t i = 0; i < items.size(); ++i)
    {
        ...
        // Item specific meet-in-the-middle patterns \mathcal{M}_i can be extracted from
        // this loop.
        const Cuboid                  &itemI = items[i];
        ...
        for(size_t p = 0; p < regularNormalPattern.size(); p++)
        {
            ...
            {
                meetInTheMiddlePoints.set((size_t)container.Dimension(axis) -
                                          (size_t)itemDimension - p);
            }
        }
...
boost::dynamic_bitset<> PlacementPointGenerator::DetermineRegularNormalPatterns(
    const Container                  &container,
    const std::vector<Cuboid *>      &items,
    const Cuboid                     &itemI,
    ...
    Axis                              axis)
{
    // Note: with rotation, item specific normal patterns can be reduced by creating item
    // specific normal patterns for each rotation separately. This is only useful, when
    // item specific placement points are actually used in the model formulation.
    int itemDimension = itemI.MinimumRotatableDimension(axis);
    ...
    for(size_t p = 0; p < regularNormalPatterns.size(); p++)
    {
        if(regularNormalPatterns[p])
        {
            ...
            {
            case PlacementPointGenerator::MeetInTheMiddleMinimizationTarget::
                IndividualPlacementPoints:
                meetInTheMiddlePointsLeft[p]++;
                meetInTheMiddlePointsRight[(size_t)container.Dimension(axis) -
                                           (size_t)itemDimension - p]++;
                break;
            case PlacementPointGenerator::MeetInTheMiddleMinimizationTarget::
                PlacementPointUnion:
...
boost::dynamic_bitset<> PlacementPointGenerator::DetermineMeetInTheMiddlePatterns(
    const Container             &container,
    const std::vector<Cuboid *> &items,
    const Cuboid                &itemI,
    int                          threshold,
    Axis                         axis)
{
    ...
    case Axis::X:
        meetInTheMiddlePoints = DetermineRegularNormalPatternsX(
            std::min(threshold - 1, container.Dimension(axis) - minDimension),
            container.Dimension(axis),
            items);
        placementPointsRightPrime = DetermineRegularNormalPatternsX(
            container.Dimension(axis) - minDimension - threshold,
            container.Dimension(axis),
            items);
        break;
    case Axis::Y:
...
boost::dynamic_bitset<> PlacementPointGenerator::DetermineRegularNormalPatternsX(
    int                          containerDx,
    int                          actualContainerDx,
    const std::vector<Cuboid *> &items)
{
    // + 1 can be neglected, because at coordinate actualContainerDx, no item with itemDx
    // > 0 can ever be placed. boost::dynamic_bitset, because of performance when building
    // set intersections in the calling methods. Instead, std::vector can also be used.
    ...
    for(size_t i = 0; i < items.size(); ++i)
    {
        ...
        for(int p = containerDx - minLength; p > -1; --p)
        {
            if(xT[p])
            {
                // + 1 --> p + item.Dx <= containerDx instead of <, cf. Algorithm 1 in
                // Côté, J. F., & Iori, M. (2018). The meet-in-the-middle principle for
                // cutting and packing problems. INFORMS Journal on Computing, 30(4),
                // 646-661.
...
boost::dynamic_bitset<> PlacementPointGenerator::DetermineRegularNormalPatternsY(
    int                          containerDy,
...
boost::dynamic_bitset<> PlacementPointGenerator::DetermineRegularNormalPatternsZ(
    int                          containerDz,
...
{
    // When the loading mask is passed from LoadingChecker, we need to use the mask and
    // cannot use the PackingType that is available there, because the loading mask might
    // be different from the PackingType.
    switch(loadingMask)
    {
    ...
    case LoadingFlag::NoLifo:
        // UD in X and Y because of support. R-NP in Z is sufficient because items must
        // not hover.
    ...
    case LoadingFlag::FragilityOnly:
        // UD in Z because items can hover, which is sometimes necessary to not violate
        // fragility or produce feasibility with hovering. R-NP in X and Y is sufficient
        // because there always exists a placement point that does not overlap with
        // another item, which retains feasibility w.r.t. fragility. MiM not feasible with
        // lifo constraints.
    ...
    case LoadingFlag::LoadingOnly:
        ...
        // Regular normal patterns with domain reduction according to Soh (2010) would
        // also be feasible.
    ...
    const std::vector<Cuboid>& items)
{
    ...
    for (size_t i = 0; i < items.size(); ++i)
    {
        ...
        int minLength = item.EnableHorizontalRotation ? std::min(item.Dx, item.Dy) :
item.Dx;
    ...
    const std::vector<Cuboid>& items)
{
    ...
    {
        const Cuboid& item = items[i];
        int minWidth = item.EnableHorizontalRotation ? std::min(item.Dx, item.Dy) :
item.Dy;
...
std::tuple<std::vector<int64>, std::vector<int64>, std::vector<int64>>
PlacementPointGenerator::DetermineNormalPatterns3D(const Container& container, const
std::vector<Cuboid>& items)
{
    auto normalPatternsX = PlacementPointGenerator::DetermineNormalPatternsX(container,
items); auto normalPatternsY =
PlacementPointGenerator::DetermineNormalPatternsY(container, items); auto normalPatternsZ
= PlacementPointGenerator::DetermineNormalPatternsZ(container, items);

    return std::make_tuple(std::move(normalPatternsX), std::move(normalPatternsY),
std::move(normalPatternsZ));
}

std::tuple<std::vector<int64>, std::vector<int64>>
PlacementPointGenerator::DetermineNormalPatternsXY(const Container& container, const
std::vector<Cuboid>& items)
{
    auto normalPatternsX = PlacementPointGenerator::DetermineNormalPatternsX(container,
items); auto normalPatternsY =
PlacementPointGenerator::DetermineNormalPatternsY(container, items);

    return std::make_tuple(std::move(normalPatternsX), std::move(normalPatternsY));
}
*/

std::vector<int64> PlacementPointGenerator::ConvertPlacementBitsetToVector(
    const boost::dynamic_bitset<> &itemSpecificPatternBitset)
{
...
ItemPlacementPatterns PlacementPointGenerator::ConvertPlacementBitsetToVector(
    const boost::dynamic_bitset<> &itemSpecificPatternBitsetX,
...
{
    return ConvertPlacementBitsetToVector(itemSpecificPatternBitset.X,
                                          itemSpecificPatternBitset.Y,
                                          itemSpecificPatternBitset.Z);
}

/*
std::vector<int64> PlacementPointGenerator::DetermineStartPointsFlatSpan(const
std::vector<int64>& patterns)
{
    ...
    // Contains the same starting points multiple times. Is need for intervals in CP
model, compare FromFlatSpanOfIntervals. startPoints.push_back(startInterval);
startPoints.push_back(patterns.back());
...
std::vector<int64>
PlacementPointGenerator::DetermineEndPoints(const std::vector<int64> &patterns,
                                            int                       dim,
                                            int                       rotDim,
                                            bool                      enableRotation)
{
...
std::vector<boost::dynamic_bitset<>>
PlacementPointGenerator::GenerateRegularNormalPatterns(const Container     &container,
                                                       std::vector<Cuboid> &items,
                                                       Axis                 axis)
{
    ...
    for(size_t i = 0; i < items.size(); i++)
    {
        ...
        case Axis::X:
            regularNormalPatterns =
                DetermineRegularNormalPatternsX(container.Dimension(axis) - itemDimension,
                                                container.Dimension(axis),
                                                filteredItems);
            break;
        case Axis::Y:
            regularNormalPatterns =
                DetermineRegularNormalPatternsY(container.Dimension(axis) - itemDimension,
                                                container.Dimension(axis),
                                                filteredItems);
            break;
        case Axis::Z:
            regularNormalPatterns =
                DetermineRegularNormalPatternsZ(container.Dimension(axis) - itemDimension,
                                                container.Dimension(axis),
                                                filteredItems);
            break;
        default:
                                                   ...
                                                   Axis             axis)
{
    ...
    case PlacementPattern::UnitDiscretization:
        itemSpecificPlacementPatterns =
            DetermineUnitDiscretizationPoints(container, items, axis);
        break;
    case PlacementPattern::RegularNormalPatterns:
        itemSpecificPlacementPatterns =
            GenerateRegularNormalPatterns(container, items, axis);
        break;
    case PlacementPattern::MeetInTheMiddle:
        ...
        itemSpecificPlacementPatterns = DetermineMinimalMeetInTheMiddlePatterns(
            container,
            items,
            MeetInTheMiddleMinimizationTarget::PlacementPointUnion,
            axis);
        break;
...
PlacementPointGenerator::GeneratePlacementPatternsBaseType(
    const Container     &container,
    std::vector<Cuboid> &items,
    PlacementPattern     placementPatternType,
    Axis                 axis)
{
    ...
    for(size_t i = 0; i < items.size(); i++)
    {
        auto itemSpecificPlacementPattern =
            PlacementPointGenerator::ConvertPlacementBitsetToVector(
                itemSpecificPlacementPatterns[i]);
        itemSpecificPlacementPatternsVector.push_back(
            std::move(itemSpecificPlacementPattern));
    }
                                                   ...
                                                   PlacementPattern placementPatternTypeZ)
{
    ...
    itemTypeSpecificPlacementPatterns.reserve(
        items.size()); // Overestimation when there are homogeneous items.
...
PlacementPointGenerator::GeneratePlacementPatternsWithUnion(
    const Container     &container,
    std::vector<Cuboid> &items,
    PlacementPattern     placementPatternTypeX,
    PlacementPattern     placementPatternTypeY,
    PlacementPattern     placementPatternTypeZ)
{
    ...
    ItemPlacementPatterns placementPointsUnion =
        PlacementPointGenerator::ConvertPlacementBitsetToVector(
            placementPointUnionBitset);

    ////std::cout << "Placement points (X, Y, Z): ";
    ////std::cout << "|X_is| = (" << itemsSpecificPlacementPointCountX << ", " <<
    /// itemsSpecificPlacementPointCountY <<
    ///", " << itemsSpecificPlacementPointCountZ << "), "; /std::cout << "|X_s| = (" <<
    /// placementPointUnionCountX << ", " << placementPointUnionCountY << ", " <<
    /// placementPointUnionCountZ << ")"; /std::cout << "\n";
    ...
    const std::vector<boost::dynamic_bitset<>> &itemSpecificPatternBitsetZ)
{
    ...
    boost::dynamic_bitset<> placementPointUnionBitsetX =
        itemSpecificPatternBitsetX.front(); // copy
    boost::dynamic_bitset<> placementPointUnionBitsetY =
        itemSpecificPatternBitsetY.front(); // copy
    boost::dynamic_bitset<> placementPointUnionBitsetZ =
        itemSpecificPatternBitsetZ.front(); // copy

    // itemSpecificPatternBitsetX.size() == itemSpecificPatternBitsetY.size() ==
    // itemSpecificPatternBitsetZ.size()
    ...
    std::cout << "|X_s| = (" << placementPointUnionCountX << ", "
              << placementPointUnionCountY << ", " << placementPointUnionCountZ << ")";
    std::cout << "\n";
}
