...
void TesteOroloc3D_NS::testeOroloc3D()
{
    ...
    strFileSol.replace(
        strFileSol.find_last_of("."), std::string::npos, "_sol_orders.csv");
    strFileSolItems.replace(
        strFileSolItems.find_last_of("."), std::string::npos, "_sol_items.csv");
    ...
    */
    ...
    while(true)
    {
        ...
        vetPontoRot[instanciaG.mapItem_IdItem[itemId]].set(
            px, py, pz, mapRotation_axisToRot[rot]);

        mapItem_idToPontRot.insert(
            {itemId, PontoRot(px, py, pz, mapRotation_axisToRot[rot])});
    ...
    auto vetPackingType = {
        PackingType::Complete}; //, PackingType::NoSupport, PackingType::LoadingOnly};
    ...
    for(auto &vet : vetVetRoutes)
    {
        ...
        // if(truckId == 2)
        {
            ...
            */
            ...
            // convertVectorOfItensToVectorOfCuboids(bin.vetItemId, vetCuboids,
            // bin.numItens);
            for(int cust : vet)
            ...
            Container      container((int)instanciaG.vetDimVeiculo[0],
            ...
            */
            ...
            for(PackingType type : vetPackingType)
            {
                ...
                auto status = loadingChecker.ConstraintProgrammingSolver(
                    type, container, stopIds, vetCuboids, 10 * 60, vetArray);
            ...
            */
            ...
            // output += std::format("{:.1f}; {}; {}", tempoCpu,
            // mapPackingTypeToString[lastType],
            // mapStatusOroloc3D_ToString[statusOroc3D]); std::cout<<output<<"\n";
...
void TesteOroloc3D_NS::convertVectorOfItensToVectorOfCuboids(
...
void TesteOroloc3D_NS::appendToFile(const std::string &fileName,
...
void TesteOroloc3D_NS::testeOroloc3D_2()
{
    ...
    auto vetPackingType = {PackingType::Complete};
        //PackingType::Complete}; //, PackingType::NoSupport, PackingType::LoadingOnly};
                ...
                if(binCp.checkFeasibility(&rota, false, true))
                {
                    ...
                    if(checkUnloadingSequence(binCp, rota))
                    {
                        ...
                        if(n == 0)
                        {
                            ...
                            {
                                output += "INFEASIBLE***; ";
                                std::printf("CP FEASIBLE*; FEASIBLE FOR LIFO SEQUENCE; "
                                            "INFEASIBLE FOR "
                                            "AXLE WEIGHTS\n");
                            }
                        }
...
void TesteOroloc3D_NS::readSolOroloc3D_2(SolucaoNS::Solucao &sol)
{
    ...
    for(int veic = 0; veic < numVeic; ++veic)
    {
        ...
        std::getline(file, str);
        std::getline(file, str);
        std::getline(file, str); // CustId	Id	TypeId	Rotated	x	y	z
                                 // Length	Width	Height	mass	Fragilit
...
void TesteOroloc3D_NS::printSol(SolucaoNS::Solucao &sol)
{
    ...
    assertm(!file.is_open(),
            "Its not possible to open the file: " << input.strSolOroloc3D_output);

    auto writeNewSeparation = [&]()
    {
        file << "\n----------------------------------------------------------------------"
                "----"
                "-----------------------------------------\n";
    ...
    for(int i = 0; i < instanciaG.numVeiculos; ++i)
    {
        ...
        if(!bin.vazio())
        {
            ...
            file << "\n\n";
            file << "CustId	Id	TypeId	Rotated	x	y	z	Length	Width	"
                    "Height	mass	Fragility	LoadBearingStrength\n";

            for(int k = 0; k < bin.numItens; ++k)
            {
                Item &item = instanciaG.vetItens[bin.vetItemId[k]];
                file << item.customer << " \t " << item.oroloc3D_item_id - 1 << " \t "
                     << item.oroloc3D_item_id;
                file << " \t " << mapRotToRotation_axis[bin.vetRotacao[k]] << " \t "
                     << (int)bin.vetPosItem[k].vetDim[0];
                file << " \t " << (int)bin.vetPosItem[k].vetDim[1] << " \t "
                     << (int)bin.vetPosItem[k].vetDim[2];
                file << " \t " << item.vetDim[0] << " \t " << item.vetDim[1] << " \t "
                     << item.vetDim[2] << " \t " << item.weight;
                file << " \t 0 \t -1\n";
            }
        }
