...
void ParseInputNS::parseInput(int argc, const char *argv[])
{
    ...
    try
    {
        ...
        options.set_width(70)
            ...
            //("s,split", "Indicate if the instance allows split delevery",
            // cxxopts::value<bool>(input.splitInstancia))
            ("alphaBin",
             "Value of the aplha parameter for the contrutive algorithm for bin packing",
             cxxopts::value<double>(input.aphaBin))
            //("aphaBinEp", "Controla a escolha do EP",
            // cxxopts::value<double>(input.aphaBinEscolhaEp))
            //("saida", "Resultado do bin packing",
            // cxxopts::value<std::string>(output.fileSaida))
            ("seed",
             "Seed of the random number generator",
             cxxopts::value<unsigned int>(output.semente))(
                "alphaVrp",
                "Value of the aplha parameter for the contrutive algorithm for VRP",
                cxxopts::value<double>(input.alphaVrp))(
                "it", "Number of iterations of IG", cxxopts::value<int>(input.numItIG))
            //("fileNum", "Contem o numero de vezes em que a instancia foi executada com
            // sucesso", cxxopts::value<std::string>(input.file.fileNum))
            //("fileSeed", "Arquivo em que a semente eh salva",
            // cxxopts::value<std::string>(input.file.fileSeed))
            ("resulCSV",
             "Arquivo em que sao escritos os resultados de cada execucao",
             cxxopts::value<std::string>(input.file.fileResulCSV))
            //("fileSol", "Arquivo em que eh escrito a solucao completa",
            // cxxopts::value<std::string>(input.file.fileSol))
            //("comprimentoAlturaIguais1", "Desabilita o bin packing, fazendo largura e
            // altura de todos os itens iguais a 1",
            // cxxopts::value<bool>(input.comprimentoAlturaIguais1))
            //("cpSat", "Abilita o cp-sat no empacotamento",
            // cxxopts::value<bool>(input.cpSat))
            //("cpSatTime", "Tempo limite para o cp-sat",
            // cxxopts::value<double>(input.cpSatTime))
            ("filo", "", cxxopts::value<bool>(input.lifo))(
                "lTaype",
                "Labeling Type (0, 1, 2)",
                cxxopts::value<int>(input.labelingType))(
                "2d",
                "Indicates if the instance is 2d. Default is 3d!",
                cxxopts::value<bool>(input.inst2d))(
                "oroloc3D",
                "Indicates if the instance is the Oroloc3D type",
                cxxopts::value<bool>(input.instOroloc3D))(
                "oroloc3D_2",
                "Indicates if the instance is the Oroloc3D_2 type",
                cxxopts::value<bool>(input.instOroloc3D_2))(
                "solOroloc3D_2",
                "File to read the instance of oroloc3D_2",
                cxxopts::value<std::string>(input.strSolOroloc3D_2))(
                "solOroloc3D_output",
                "File to read the instance of oroloc3D_2",
...
void ParseInputNS::escreveFileResulCsv(const SolucaoNS::Solucao &sol,
                                       ...
                                       double                    tempoCpu)
{
    ...
    file << sol.distTotal << ", " << tempoCpu << ", " << sol.getUtilizacaoMedianaBins()
         << ", " << sol.getTamMedianaRota() << ", " << output.semente << "\n";
    file.close();
}
...
void ParseInputNS::escreveFileSol(const SolucaoNS::Solucao &sol,
                                  const std::string        &fileSol)
{
...
void ParseInputNS::escreveFileSolPrint(const SolucaoNS::Solucao &sol,
                                       const std::string        &fileSol)
{
