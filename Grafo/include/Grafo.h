...
template <typename T> void copyGraph(const Graph<T> &src, Graph<T> &dest)
{
...
template <typename T> std::string printGraph(const Graph<T> &graph)
{
    std::string str;
    ...
    for(int64_t i = 0; i < graph.numVertices; ++i)
    {
        for(const auto &it : graph.arcs[i])
            str += "(" + std::to_string(i) + ", " + std::to_string(it.first) +
